...
export const FindProductByPrice = (min: number, max: number) => {
  return intansce.get(
    `/products/filter/price?minPrice=${min}&maxPrice=${max}&sortType=desc`
  );
};
export const ProductSort = (value: string) => {
  if (value === null || value === undefined) {
    return intansce.get("/products");
  }
  return intansce.get("/products/sortyes?sort_name=" + value);
};
