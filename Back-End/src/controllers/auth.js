...
export const Signup = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      message:
        "Đăng ký thành công. Vui lòng kiểm tra email của bạn để xác thực tài khoản và đăng nhập",
      user,
    });
  } catch (error) {
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const newAccessToken = jwt.sign({ _id: user._id }, SECRET_CODE, {
      expiresIn: "7d",
    });
    ...
    if (error.name === "Token hết hạn") {
      return res.status(401).json({
        message: "Mã Token làm mới đã hết hạn, vui lòng đăng nhập lại",
      });
    }
