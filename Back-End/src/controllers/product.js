...
// products by name 'products/sortyes?sort_name=az or za'
export const ProductSortByName = async (req, res) => {
  let checkSort = req.query.sort_name;
  if (!checkSort) return;
  let options = {
    name: 1,
  };
  ...
  try {
    const products = await Product.find().sort(options);
    if (products.length === 0) {
      return res.status(404).json({
        message: "Không có sản phẩm bạn muốn tìm",
      });
